...
if (!($customer_firstname && $customer_lastname && $customer_email)) {
   ...
   $url = "http://".$_SERVER['HTTP_HOST']."/Lesson14-PHP/register5.php?".$query_string."&error=1";
   header("Location: ".$url);
...
$subtotal = 0;
  ...
  $item_number = $_SESSION['cart'][$id];    
  $product_price = $line_item_price * $quantity;
  
  $email_message .= "Title: ".$title."\n";
  $email_message .= "Price: $".number_format($line_item_price,2)."\n";
  $email_message .= "Quantity: ".$item_number."\n";
...
/* This is where I might try to empty the cart
